...
import org.liangjiakun.teris.object.impl.PieceCreatorImpl;
...
public class MainFrame extends JFrame {
	...
	public void right(int size) {
		...
		// 判断是否超过GamePanel的宽
		if (this.currentPiece.getMaxXLocation() 
				>= this.gamePanel.getWidth()) return;
	...
	private boolean isLeftBlock() {
		...
		for (int i = 0; i < squares.size(); i++) {
			...
			// 查找界面数组中的Square对象
			Square block = getSquare(s.getBeginX() - Piece.SQUARE_BORDER, 
					s.getBeginY());
			// block非空表示遇到障碍
